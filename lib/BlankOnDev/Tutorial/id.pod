...
Untuk tutorial dalam bentuk PDF dapat di download di : L<https://yusrideb.github.io/BlankOnDev/docs/Tutorial-BlankOnDev-01005.pdf>


=head1 1. Pengenalan

=head2 1.1. Apa itu BlankOnDev Tools ?

B<BlankOnDev> merupakan Tools untuk melakukan aktifitas sebagai pengembang GNU/Linux BlankOn.
Terdapat beberapa pekerjaan sebagai pengembang BlankOn khususnya untuk B<Tim Pemaket> yang dilakukan dengan cara manual menggunakan beberapa perintah Linux, yang menurut penulis cukup tradisional.

Pada tools B<BlankOnDev> I<rencana> akan memiliki beberapa fitur untuk mempermudah pekerjaan Tim Pemaket meliputi :

=over

=item 1.
Manajemen pengaturan repositori pada Sistem Linux Pemaket

=item 2.
Persiapan Sistem Linux Pemaket sebelum melakukan aktifitas pemaket.

=item 3.
System manajemen builder Paket dari Source menjadi file B<.deb> yang siap untuk di install dengan perintah C<dpkg -i [file_packages.deb]>.

=item 4.
Melakukan patching atau perbaikan bugs yang terpadat pada paket-paket tertentu.

=item 5.
Membuat format paket debian yang nantinya akan diteruskan ke server B<irgsh> untuk dilakukan proses build source
menjadi paket debian yaitu file B.deb>, yang kemudian akan di dorong ke B<reprepo>.

=item 6.
Migrasi data paket debian yang sebelumnya tersimpan dalam server bazaar repository ke server github repository.

=item 7.
Sistem pembuatan Repositori mirror secara otomatis dengan sistem singkronisasi yang tercontrol.

=back

Fitur B<BlankOnDev v0.1005> yang sudah rampung dan siap untuk digunakan :
...
=head2 1.2. Mengapa BlankOnDev Harus dibuat ?

Ada beberapa aktifitas yang mememakan banyak waktu jika harus dilakukan dengan cara tradisional di Sistem Operasi GNU/Linux.
GNU/Linux BlankOn merupakan Distribusi Linux turunan Debian GNU/Linux, dengan basis Debian GNU/Linux sebenarnya
sudah memberikan banyak kemudahan dalam penggunaannya, akan tetapi tetap memakan waktu yang banyak.
Sedangkan ada hal yang lebih penting daripada sekedar menjalankan perintah linux satu per satu yaitu
Kestabilan dari Distribusi Sistem Operasi GNU/Linux.

Berikut beberapa kasus yang menginspirasi penulis untuk membuat Tools dengan nama B<BlankOnDev>, yaitu :

=over

=item * Konfigurasi sistem lokal pemaket sebelum melakukan aktifitas pemaket, ini juga harus dilakukan banyak percobaan
untuk menghasilkan konfigurasi yang stabil.

=item * Sistem kontrol server repositori yang menurut penulis masih perlu diperbaiki untuk mencegah terjadi permasalahan
di server repositori GNU/Linux BlankOn. Dan ketika terjadi permasalahan dapat diselesaikan dalam kurung waktu sangat singkat.

=item * Build Paket di mesin lokal Pemaket. Aktifitas ini, juga bisa dikatakan sangat panjang jika terjadi permasalahan seperti
patch bugs paket BlankOn. Aktifitas build tanpa adanya permasalahan juga akna sangat panjang, jika Pemaket harus menentukan
versi paket yang akan dipaketkan atau melakukan uji coba terhadap versi paket sebelum di dorong ke B<irgsh>.

=item * Build paket beserta dependensi paket. Setiap paket utama yang akan dibuild tentunya memiliki dependensi atau
dalam artian adanya keterkaitan antar paket lainnya. Agar paket utama yang dibuild dapat stabil saat installasi maupun penggunaannya,
maka dependensi paket juga harus sama stabilnya. Kemudian, jika hal ini dilakukan secara manual oleh pemaket,
maka pasti akan memakan banyak waktu.

=item * Pengaturan repositori di sistem pemaket, proses penggantian alamat Repositori masih tergolong manual,
karena harus mengedit file konfigurasi di C</etc/apt/sources.list>. Jika hanya sebagai pengguna biasa ini tidak masalah.
Namun untuk pengembang ini sangat banyak makan waktu ketika harus melakukan pengujian terhadap repositori.
...
Selain butir-butir diatas penulis juga terinspirasi dengan B<Debian Manifasto> yang di tulis oleh I<Ian A. Murdock>,
sebagai konstruksi awal dari pengembangan Debian GNU/Linux. Dan juga menjadi landasan pengembangan I<BlankOnDev Tools>.

=head2 1.3. Teknologi yang digunakan oleh BlankOnDev

Pengembangan B<BlankOnDev tools> menggunakan bahasa pemrogram B<Perl> demi kemudahan penulis untuk membuat beberapa tools,
apalagi Perl sangat powerful untuk pemrograman text.

Terdapat beberapa paket-paket Debian yang masih menggunakan I<Perl> sampai saat ini, seperti beberapa tools I<dpkg>, I<apt>
dan masih banyak lagi tool-tool yang selama ini sering digunakan dan dibuat dengan bahasa pemrograman B<Perl>.

I<Perl philosophy : "Kami dapat mengajari Anda bagaimana kami melukis, tapi kami tidak dapat mengajari Anda bagaimana Anda melukis">

Prototype B<apt-get> dan Baptitude> juga awalanya menggunakan bahasa pemrogram Perl yang kemudian ditulis ulang ke C/C++.
Tools B<tasksel> yang berjalan pada instalasi mode text Debian GNU/Linux dan juga bisa dijalakan setelah instalasi, masih menggunakan Perl.
Beberapa hal ini juga menjadi alasan penulis untuk membuat program BlankOnDev dengan bahasa pemrogram perl.
Konsep seperti tool B<tasksel> yang akan digunakan oleh penulis kedepannya untuk memenuhi seluruh daftar fitur yang disebutkan pada I<Tutorial> ini.

Salah satu tools Debian yang digunakan untuk melihat semua dependensi terhadap suatu paket tertentu yaitu B<apt-rdepends>.
output dari tools dari B<apt-rdepends> seperti berikut :
...
=item 14.
Perintah C<B<boidev bzr2git git-push-new>> - Perintah ini merupakan perintah yang digunakan untuk mendorong/upload yang sudah didownload namum belum dikonversi ke format repositori github, berdasarkan nama paket yang sudah terdaftar pada system program B<BlankOnDev>. Perintah ini sama seperti instruksi yang tampil dihalaman github saat  nama repositori baru ditambahkan.
...
=item 1.
Perintah C<B<boidev help>> - Untuk melihat seluruh help perintah C<boidev>
...
Pada bagian ini sebagai contoh file I<Public Key SSH> berada di C</home/yusrideb/.ssh/id_rsa.pub>
Setelah itu, masuk ke akun github dan lakukan pengaturan di bagian I<SSH and GPG Keys> untuk menanbahkan I<SSH Key> pada B<Github> dengan isi dari file
C</home/yusrideb/.ssh/id_rsa.pub>, seperti berikut :

=begin html

<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/add-ssh-key-on-gtihub.png" alt="Form Awal CPAN" />
<figcaption>Gambar 2.1. <b>Public SSH Key> di Akun Github</b></figcaption>
</figure>
</center></p>
...
=item * Klik B<SSH and GPG Keys>

=item * Klik B<New SSH Key>

=item * Masukkan Nama I<SSH Key>, contohnya B<My_SSH\_key>

=item * Masukkan isi file C<id\_rsa.pub> ke dalam form input B<Key>, Setelah itu klik B<Add SSH Key>.

=item * Hasil akan tampil di Daftar B<SSH Keys> seperti yang ditunjukkan oleh gambar diatas.

=back

=head3 2.1.3. Metode Instalasi secara Manual

Instalasi manual untuk Module Perl B<BlankOnDev> yang dimaksud disini yaitu instalasi dari Source Module CPAN yang dapat didownload pada halaman :

L<https://metacpan.org/module/BlankOnDev> atau L<https://github.com/yusrideb/BlankOnDev>

=head4 2.1.3.1. Install Dependensi Module

Jika versi B<BlankOnDev Toos> yang terbaru sudah pernah di install sebelumnya, maka I<Dependensi> tidak pernah di install lagi.
...
=head4 2.1.3.1. Install Module Perl

Download Source dari halaman L<https://metacpan.org/module/BlankOnDev>,
Contoh file source : "B<BlankOnDev-0.1005.tar.gz>"

    tar xzvf BlankOnDev-0.1005.tar.gz
    cd BlankOnDev-0.1005
    sudo perl Makefile.PL && make && make install && make clean.

=head3 2.1.4. Metode Instalasi BlankOnDev dari CPAN Perl

Pada bagian ini I<Dependensi> beserta module utama akan di install secara otomatis. ketika perintah C<cpan -i> di jalankan, seperti berikut :

    sudo cpan -i BlankOnDev

Proses instalasi ini akan berlangsung kurang lebih 10 menit dan bisa lebih tergantung kemampuan PC/Laptop Anda, karena selama proses instalasi berlangsung terdapat beberapa module-module dependensi yang melakukan kompilasi I<C Source>.

Dengan instalasi menggunakan perintah C<cpan -i>, maka versi yang akan terinstalasi dari module dependensi atau module utama B<BlankOnDev> yaitu versi terbaru, tidak seperti B<Metode instalasi manual>, dimana versi module tergantung versi yang Anda download.

Jika belum pernah menjalankan perintah B<C<cpan -i>> sebelumnya, maka pada saat perintah tersebut dijalankan pertama kali akan tampi B<Form> seperti berikut :

=begin html

<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/install-from-cpan-1.png" alt="Form Awal CPAN" />
<figcaption>Gambar 2.2. <b>Form Awal CPAN</b></figcaption>
</figure>
</center></p>

=end html

Jika tampilkan form seperti gambar diatas, langsung saja tekan B<Enter>.

Jika sebelumnya anda sudah menggunakan perintah B<C<cpan -i>> maka instalasi module Perl akan dimulai, seperti berikut :

=begin html

<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/install-from-cpan-2.png" alt="Form Awal CPAN" />
<figcaption>Gambar 2.3. <b>Instalasi Module Perl di Mulai</b></figcaption>
</figure>
</center></p>
...
<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/install-from-cpan-3_finish.png" alt="Form Awal CPAN" />
<figcaption>Gambar 2.4. <b>Instalasi Module Selesai</b></figcaption>
</figure>
</center></p>

=end html

=head2 2.2. Implementasi

Pada bagian ini akan dijelaskan langkah-langkah penggunaan perintah B<boidev>. Berikut gambaran penggunaan perintah B<boidev> :

=head3 2.2.1. Persiapan Migrasi Paket :

=over

=item 1.
B<Step 1> - Perintah yang dijalankan sebelum menjalankan perintah B<boidev> lainnya yaitu perintah B<C<boidev config>> untuk konfigurasi program seperti :

=over

=item * Memasukkan nama B<time zone>,

=item * Memasukkan alamat B<Email> yang digunakan di akun I<Github> dan maupun alamat B<Email> untuk generate key dengan GnuPG, dan
...
=item 2.
B<Step 2> - Menggunakan perintah C<boidev mig_prepare>. Perintah ini akan melakukan persiapan sebelum Migrasi paket, yaitu :

=over

=item * Config github, seperti I<nama>, I<Alamat Email>, dan I<Cache Username dan Password>

=item * Opsi Generate Key GnuPG.

=item * Memasukkan alamat bazaar repositori (example: B<http://dev.blankonlinux.or.id/browser/tambora>).

=item * Memasukkan alamat github repositori (example: B<https://github.com/blankon-packages>)

=back

=item 3.
B<Step 3> -  Menambahkan I<Group Paket> untuk I<Paket-paket> yang akan di migrasi ke repositori github. Perintah yang digunakan antara lain :

=over

=item * perintah C<boidev bzr2git addpkg-group>,

=item * perintah C<boidev bzr2git addpkg> dan

=item * perintah C<boidev bzr2git addpkg-file>
...
\noindent
Setelah form diatas, maka akan tampil hasil B<Generate Key GnuPG> :
...
=item * Jika jawaban B<C<n>>, maka akan langsung ke form untuk memasukkan URL.

=back

=head4 2.3.2.3. Form URL Repositori Bazaar dan Github :

=over

=item * Jika data B<belum ada> :
...
=head3 2.3.3. Menambahkan Group paket dan nama paket

Seperti yang telah dijelaskan pada upabab L<I<2. Implementasi - I<Persiapan Migrasi Paket>>|https://metacpan.org/pod/distribution/BlankOnDev/lib/BlankOnDev/Tutorial/id.pod#Proses-Migrasi-Paket> yaitu menggunakan perintah "C<boidev bzr2git addpkg-group>", "C<boidev bzr2git addpkg>" dan "C<boidev bzr2git addpkg-file>".

=head4 2.3.3.1. Menambahkan Group Paket
...
Atau langsung masukkan nama paket group, contoh nama group paket B<githb-6>
...
=head4 2.3.3.2. Melihat daftar Group paket
...
=head4 2.3.3.3. Menambahkan nama paket
...
=head4 2.3.3.4. Menambahkan nama paket dari file
...
Daftar nama paket tersebut dimasukkan kedalam file yang berekstensi B<.boikg>, karena program hanya akan membaca file yang berekstensi B<.boikg>

Setelah daftar I<nama paket> dimasukkan dalam file, kemudian jalankan perintah berikut dengan syntax : C<boidev bzr2git addpkg-file [lokasi_file/nama_file.bokg]>

Contoh lokasi file di : C</home/yusrideb/github-6.boikg>
...
=head4 2.3.3.5. Melihat Daftar Paket

Untuk melihat list paket, menggunakan perintah :

=over

=item * C<boidev bzr2git list-pkg>

=item * C<boidev bzr2git list-pkg <nama_group_paket>>

=back

B<Perintah C<boidev bzr2git list-pkg>>
...
<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/boidev_bzr2git_list-pkg.png" alt="Daftar Paket yang telah ditambahkan dengan nama group github-6" />
<figcaption>Gambar 2.5. <b>Daftar Paket yang telah ditambahkan dengan nama group github-6</b></figcaption>
</figure>
</center></p>

=end html

B<Perintah C<boidev bzr2git list-pkg <nama_group_paket>>
...
<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/boidev_bzr2git_list-pkg_groupName.png" alt="Daftar Paket yang telah ditambahkan dengan nama group github-6" />
<figcaption>Gambar 2.6. <b>Daftar Paket yang telah ditambahkan dengan nama group github-6</b></figcaption>
</figure>
</center></p>

=end html

=head2 2.4. Proses Migrasi Paket

Proses yang dilakukan disini yaitu penggunaan perintah-perintah seperti contoh berikut. Perintah B<boidev> bertujuan untuk meminimalisir kesalahan penulisan perintah ataupun kesalahan penulisan I<nama paket>.
...
=head3 2.4.1. Migrasi Paket dengan B<Metode Split>

Seperti yang telah dijelaskan pada bagian I<2. Implementasi> diatas, Bahwa I<metode> ini melakukukan I<branch paket>, I<convert format ke git>,
dan I<push ke git> secara terpisah. Seperti metode migrasi yang dilakukan secara manual seperti yang ditunjukkan pada script diatas.
Berikut langkah-langkahnya :

=head4 2.4.1.1. Perintah C<boidev bzr2git branch>

Perintah yang dapat digunakan yaitu :

=over

=item * C<boidev bzr2git branch>

=item * C<boidev bzr2git branch <nama\_group\_paket>>
...
=head4 2.4.1.2. Perintah C<boidev bzr2git bzr-cgit>

Perintah yang dapat digunakan yaitu :

=over

=item * C<boidev bzr2git bzr-cgit>

=item * C<boidev bzr2git bzr-cgit <nama_group_paket>>
...
=head4 2.4.1.3. Perintah C<boidev bzr2git git-push>

Perintah yang dapat digunakan yaitu :

=over

=item * C<boidev bzr2git git-push>

=item * C<boidev bzr2git git-push [nama_group_paket]>
...
=head4 2.4.1.5. Perintah C<boidev bzr2git git-check>

Perintah yang dapat digunakan yaitu :

=over

=item * C<boidev bzr2git git-check>

=item * C<boidev bzr2git git-check <nama_group_paket>>
...
=head4 2.4.3.3. Melihat hasil Migrasi

Jika anda ingin melihat hasil migrasi gunakan perintah C<boidev bzr2git list-pkg [nama_group_paket]> seperti berikut :
...
<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/boidev_bzr2git_list-pkg-check.png" alt="List Packages" />
<figcaption>Gambar 2.7. <b>List Packages</b></figcaption>
</figure>
</center></p>

<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/boidev_bzr2git_list-pkg-check_zoom.png" alt="List Packages bagian hasil git-check" />
<figcaption>Gambar 2.8. <b>List Packages bagian hasil git-check</b></figcaption>
</figure>
</center></p>
...
=head3 2.4.2. Migrasi Paket dengan B<Skema One-time>

Seperti yang telah dijelaskan pada upabab L<I<2. Implementasi - Proses Migrasi Paket - Skema one-time>|https://metacpan.org/pod/distribution/BlankOnDev/lib/BlankOnDev/Tutorial/id.pod#Proses-Migrasi-Paket>,
Bahwa I<metode> ini melakukukan I<branch paket>, I<convert format ke git>, dan I<push ke git> sekaligus dengan perintah I<C<boidev bzr2git>>. Berikut langkah-langkahnya:

=head4 2.4.2.1. Migrasi 1 paket
...
=head4 2.4.2.2. Migrasi Berdasarkan group paket
...
=head4 2.4.2.3. Migrasi Semua paket dalam list
...
=head1 3. Troubleshooting Migrasi Paket

Pada bagian ini akan dijelaskan 2 permasalah terkait dengan aktifitas Migrasi paket dari Repositori Bazaar ke Repositori Github.
Beberapa permasalahan yang akan dijelaskan pada upabab ini, akan diselesaikan dengan perintah C<boidev bzr2git> dan C<boidev bzr2git [cmd2]>.

=head2 3.1. Paket github belum dikonverersi
...
=head3 3.1.1. Problem Solved dengan perintah C<boidev bzr2git [cmd]>

Pada contoh ini nama paket yang bermasalah yaitu B<notify-osd>, berikut daftar perintah yang digunakan untuk menyelesaikan masalah :

=over

=item * Perintah C<boidev bzr2git branch B<notify-osd>>

=item * Perintah C<boidev bzr2git bzr-cgit B<notify-osd>>

=item * Perintah C<boidev bzr2git re-gitpush B<notify-osd>>

=item * Perintah C<boidev bzr2git git-check B<notify-osd>>
...
<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/solved-github-no-convert.png" alt="Penyelesaian masalah : Format paket belum dikonversi ke format gitb" />
<figcaption>Gambar 3.2. <b>Penyelesaian masalah : Format paket belum dikonversi ke format git</b></figcaption>
</figure>
</center></p>

=end html

=head3 3.1.2. Problem Solved dengan perintah C<boidev bzr2git>

Pada bagian ini akan diilustrasi penyelesaian masalah mengunakan perintah C<boidev bzr2git>. Berikut Ilustrasinya :
...
<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/solved-github-no-convert.png" alt="Penyelesaian masalah : Format paket belum dikonversi ke format git" />
<figcaption>Gambar 3.3. <b>Penyelesaian masalah : Format paket belum dikonversi ke format git</b></figcaption>
</figure>
</center></p>

=end html

=head2 3.2. Paket github hanya memiliki 1 jenis I<Branch>

Berikut contoh paket yang di dorong ke github, namun tidak dibuatkan branch ke rilis B<tambora>, hanya branch B<master> saja.
...
Untuk memperbaiki repositori github seperti permasalah ini, dapat diselesaikan dengan metode yang digunakan yaitu B<Metode Split> atau B<Metode One-time>.
Pada bagian ini menggunakan B<Metode One-time>. Berikut ilustrasinya :
...
<p><center>
<figure>
<img src="https://yusrideb.github.io/BlankOnDev/docs/source/img/solved-github-1-branch.png" alt="Penyelesaian Masalah : Repositori Github hanya memiliki type branch I<master>" />
<figcaption>Gambar 3.5. <b>Penyelesaian Masalah : Repositori Github hanya memiliki type branch I<master></b></figcaption>
</figure>
</center></p>
...
=item * Achmad Yusri Afandi, E<lt>yusrideb@cpan.orgE<gt>

=item * BlankOn Developer Team.
